...
def fit_gpytorch_manifold(
    ...
    solver: Optimizer = pyman_solvers.conjugate_gradient.ConjugateGradient(),
    nb_init_candidates: int = 20,
    ...
    if np.random.rand() < last_x_as_candidate_prob:
        x0_candidates = [x0]
        x0_candidates += [manifold.random_point() for i in range(nb_init_candidates - 1)]
    else:
        x0_candidates = []
        x0_candidates += [manifold.random_point() for i in range(nb_init_candidates)]
    ...
    with gpt_settings.fast_computations(log_prob=approx_mll):
        ...
        solver._verbosity = 0
        # Solve
        optimizerobject = solver.run(problem, initial_point=x_init)
        opt_x = optimizerobject.point
        opt_log = optimizerobject.log
        #opt_x, opt_log = solver.run(problem, initial_point=x_init)

    # Construct info dict
    info_dict = {
        "fopt": float(cost(opt_x).detach().cpu().numpy()),
