...
def run_experiemt(args):
    ...
    if args.test_func == "Lasso":
        objective = test_functions.lasso_hard(device=device)
        D = 1000
    elif args.test_func == "Lasso_Leukemia":
        objective = test_functions.lasso_real_proposed(device=device)
        D = 7129

    prefix_init = root_dir + "/initial_file/Lasso/"
    filename_init = prefix_init + str(args.test_func) +'_results_CS' + '_D' + str(D) + '_n' + str(
    ...
    if os.path.exists(filename_init):
        ...
        initial_points_high = np.empty((args.rep, args.initial_n, D))
    ...
    for i in range(args.rep):
        best_min = 1e8
        proposed_model = GuA_NN_model(d_orig = D, d_embedding = args.proj_dim, hidden_units=args.hidden_units, 
...
if __name__ == "__main__":
    ...
    parser.add_argument('--test_func', type=str, default="Lasso", choices=["Lasso","Lasso_Leukemia"])
