...
import pickle
import torch
from models.geometry_aware_model import GA_sphere_model
...
def run_experiemt(args):
    ...
    if args.test_func == "Ackley_Sphere_1":
        objective = test_functions.ackley_proposed_sphere(args.effective_dim, device)
    elif args.test_func == "Hyper_Sphere_1":
        objective = test_functions.hyper_proposed_sphere(args.effective_dim, device)
    
    result_f_s = []
    for i in range(args.rep):
        best_min = 1e8
        proposed_model = GA_sphere_model(d_orig = args.high_dim, d_embedding = args.proj_dim, d_embedding_sphere=args.effective_dim+1, 
...
if __name__ == "__main__":
    ...
    parser.add_argument('--test_func', type=str, default="Ackley_Sphere_1", choices=["Ackley_Sphere_1", "Hyper_Sphere_1"])
    ...
    parser.add_argument('--effective_dim', type=int, default=10)
